...
# Full RL training run: VLM RL on VisGym maze_2d/hard with Qwen3-VL-2B.
#
...
#
# Usage:
#   bash examples/train/visgym/run_visgym_full_maze3d_hard.sh

: "${ENV_ID:=maze_2d/hard}"
: "${DATA_DIR:="$HOME/data/visgym_maze2d_hard"}"
: "${NUM_INFERENCE_GPUS:=4}"
: "${NUM_TRAIN_GPUS:=4}"
: "${LOGGER:=wandb}"

: "${EXPORT_PATH:="$HOME/exports/visgym_maze2d_hard"}"
...
: "${EVAL_DATA_DIR:="$HOME/data/visgym_maze2d_hard_eval"}"
...
uv run --isolated --extra fsdp \
  ...
  trainer.project_name="vlm_maze2d_hard" \
  trainer.run_name="maze2d_hard_full-20-turn" \
  ...
  trainer.ckpt_path="$HOME/ckpts/visgym_maze2d_hard-lora" \
